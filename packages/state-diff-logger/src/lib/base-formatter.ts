...
class BaseFormatter {
  ...
  finalize({ buffer, styles }) {
    if (isArray(buffer)) {
      return [buffer.join(''), ...styles];
    }
  }
  ...
  rootBegin(context: any, type: string, nodeType: string) {
  ...
  rootEnd(context: any, type: string, nodeType: string) {
    throw new Error('Method not implemented.');
  }
  nodeEnd(
    context: any,
